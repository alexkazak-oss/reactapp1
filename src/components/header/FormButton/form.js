...
const HeaderForm = () => {
	...
	useEffect(() => {
		const handleOutsideClick = event => {
			const filePreviewVisible = document.querySelector('#form')
			if (
				formRef.current &&
				!formRef.current.contains(event.target) &&
				!event.target.closest('#file-preview') &&
				!event.target.closest('#remove-iconSvg') &&
				!filePreviewVisible.contains(event.target) &&
				isVisible
			) {
				setIsVisible(false)
	...
	const sendEmail = (selectedOptions, selectedFiles) => {
		// идентификаторы сервиса и шаблона
		const serviceId = 'service_us5aeus' //service id
		...
		emailjs
			.send(serviceId, templateId, data, '9v6iGpGPnIZhToP11')
			.then(response => {
	...
	const onSubmitForm = (formData, selectedFiles) => {
	...
	const handleSubmit = async event => {
		...
		if (!isEmailValid) {
			...
			console.log('Невалидный email. Форма не отправлена.')
			return onSubmitForm(formData, selectedFiles)
		}
		console.log('Данные формы:', formData)

		// Отправка данных формы по электронной почте
		onSubmitForm(formData, selectedFiles)
	}
	...
	const handleKeyDown = event => {
		if (event.key === 'Tab') {
			const focusableElements = formRef.current.querySelectorAll(
				'input, textarea, button, .form, .select-button, .options-container, input'
			)
